...
function getPanLimits(
  ...
  const maxTx = Math.max(0, (scaledW - vpW) / 2);
  const maxTy = Math.max(0, (scaledH - vpH) / 2);
...
export function ZoomableBoardViewport({
  ...
  const pinch = Gesture.Pinch()
    ...
    .onEnd(() => {
      ...
      if (scale.value <= MIN_SCALE) {
        ...
        const fitsX = (boardWidth * scale.value) <= viewportW.value;
        const fitsY = (boardHeight * scale.value) <= viewportH.value;
        if (fitsX && fitsY) {
          translateX.value = withTiming(0);
          translateY.value = withTiming(0);
          savedTranslateX.value = 0;
  ...
  const pan = Gesture.Pan()
    ...
    .onUpdate((e) => {
      const canPanX = (boardWidth * scale.value) > viewportW.value;
      const canPanY = (boardHeight * scale.value) > viewportH.value;
